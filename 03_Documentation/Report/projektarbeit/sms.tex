...
\begin{figure}[h]
    ...
    \subfigure[SOES application state machine]{\label{subfig:soes_sm}{
        ...
        (SOES_TIMER)    edge node [anchor=center,above]{$timeElapsed$}                 (SOES_SLAVE)
...
\begin{figure}[h]
    ...
    \subfigure[Synchronization state machine]{\label{subfig:ecat_sm}{
       ...
       % STATE T_START 
        ...
        % State: T_RUNNING
        \node[state,    	% layout (defined above)
            text width=3.8cm, 	% max text width
            yshift=2cm, 		% move 2cm in y
            right of=T_CONFIG, 	% Position is to the right of QUERY
            node distance=5cm, 	% distance to QUERY
            anchor=center] (T_RUNNING) 	% posistion relative to the center of the 'box'
        {%
        \begin{tabular}{l} 	% content
            \textbf{Running}\\[.1em]
            \parbox{4cm}{
        ...
        % draw the paths and and print some Text below/above the graph
        \path
            (T_START)       edge node[anchor=center,left]{$osOk$}  (T_CONFIG) 
            (T_CONFIG) 	    edge[bend left=20]  node[anchor=center,left]{$sys\_event!$} (T_RUNNING)
            (T_RUNNING)     	edge[bend left=20] node[anchor=north,above]{$fault?$} (T_FAULT)
            (T_FAULT)       	edge[bend left=30]  node[anchor=center,right]{$timeElapsed$}     (T_RESTART)
            (T_RESTART)       	edge[bend left=30] node[anchor=center,left]{$done$}    (T_CONFIG)
            ;
  
        \end{tikzpicture}
    }}\hfill
    \subfigure[Temperature application state machine]{\label{subfig:soes_sm}{
        %SOES state machine 80/100
        \begin{tikzpicture}[->,>=stealth']
        % STATE APP_INIT
        % Use previously defined 'state' as layout (see above)
        ...
        % STATE SOES APP_SEARCH
        \node[state,    	% layout (defined above)
            text width=3.2cm, 	% max text width
            %yshift=2cm, 		% move 2cm in y
            below of=APP_INIT, 	
            node distance=3cm, 	% 
            anchor=center] (APP_SEARCH) 	% posistion relative to the center of the 'box'
        {%
        \begin{tabular}{l} 	% content
            \textbf{Search}\\[.1em]
            \parbox{2.8cm}{
        ...
        \node[state,
        text width = 3.2cm,
        right of=APP_REQUEST,
        %xshift= 2.5cm,
        node distance=5cm,
        anchor=center] (APP_WAIT) 
        {%
        \begin{tabular}{l}
        \textbf{Wait}\\[.1em]
        \parbox{4cm}{
        ...
        \path
        (APP_START)    edge node[anchor=center,above]{$resume?$} (APP_INIT) 
        (APP_INIT) 	edge  node[anchor=south,right]{$done$}  (APP_SEARCH)
        (APP_SEARCH)    edge [loop below]   node[anchor=east,below]{$noDevices$} (APP_SEARCH)
        (APP_SEARCH)    edge node [anchor=center,above]{$found$} (APP_REQUEST)                                          (SOES_TIMER)
        (APP_REQUEST)    edge node [anchor=center,above]{$done$}                   (APP_WAIT)
        (APP_WAIT)    edge [bend left = 50]   node[anchor=center,right]{$delayDone$}   (APP_READ)
        (APP_READ)    edge [bend left = 50]   node[anchor=center,left]{$done$}   (APP_REQUEST)
        ;
  
        \end{tikzpicture}
    }}
    \caption{State machines for Temperature functionality}  %EtherCAT Device Protocol poster from EtherCAT resources
    \label{fig:sm_temperature}
  \end{figure}
